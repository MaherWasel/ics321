...
class AdminRepositroy {
  ...
  Future<List<FlightModel>> getBookedPercentagesForFlights(
    ...
    for (int i = 0; i < filteredList.length; i++) {
      final response = await Supabase.instance.client
          .from("Ticket")
          .select()
          .eq("flight_id", filteredList[i].id)
          .not("status", 'eq', 'Cancelled')
          .not("status", 'eq', 'waitList');
  ...
  Future<List<Plane>> getPlaneReports({required DateTime date}) async {
    ...
    for (int i = 0; i < filteredList.length; i++) {
      final planeResponse = await Supabase.instance.client
          .from("Plane")
          .select()
          .eq("id", filteredList[i].planeId);
      final plane = Plane.fromMap(planeResponse[0]);
      if (!listOfPlanes.contains(plane)) {
        final ticketReponse = await Supabase.instance.client
            .from("Ticket")
            .select()
            .eq("flight_id", filteredList[i].id)
            .not("status", 'eq', 'Cancelled');
  ...
  Future<List<Ticket>> getTicketsForFlight(
      {required FlightModel flight}) async {
    final response = await Supabase.instance.client
        .from("Ticket")
        .select()
  ...
  Future<List<Ticket>> getWaitListedTickets(
      {required FlightModel flight}) async {
    final response = await Supabase.instance.client
        .from("Ticket")
        .select()
